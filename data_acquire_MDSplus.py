...
def main(shot,timebase,path_shot):
    ...
    while True:
	    ...
	    break
        except (TreeNODATA,TreeFOPENR,TreeNNF): #not available if LH turned off
            p_lh = zeros
            time_p_lh = timebase
    ...
    while True:
        try:
            DNB = MDSplus.Tree('dnb', shot)
            i_beam = DNB.getNode('\DNB::I_BEAM').data()
            time_i_beam = DNB.getNode('\DNB::I_BEAM').dim_of().data()
            i_beam = np.interp(timebase,time_i_beam,i_beam,left=np.nan,right=np.nan)
	    break
        except (TreeNODATA,TreeFOPENR,TreeNNF): #not available if LH turned off
            i_beam = zeros
            time_i_beam = timebase
            print("No values stored for i_beam") 
            print(shot)
            break
        except:
	    if i_beam == 0.0:
                i_beam = zeros
                time_i_beam = timebase
		print("i_beam was just element 0")
		break
	    else:
                print("Unexpected error for i_beam")
                print(shot)
                raise             
    ...
    while True:
        try:
            ...
            freq_icrf_d = freq_icrf_e = freq_icrf_j = NaN
            print("No values stored for p_icrf_freq") 
            print(shot)
            break
        except:
            print("Unexpected error for p_icrf_freq")
            print(shot)
            raise
    ...
    while True:
        try:  
            ...
            rgap = (efit.getNode('\efit_aeqdsk:oright')).data()[good_time_slices]/100.
            rgap = np.interp(timebase,time_efit,rgap,left=np.nan,right=np.nan)
            lgap = (efit.getNode('\efit_aeqdsk:oleft')).data()[good_time_slices]/100.
            lgap = np.interp(timebase,time_efit,lgap,left=np.nan,right=np.nan)
...
#            raise
    ...
    np.savez('/home/mathewsa/Desktop/single_shot_training_table_py.npz', ip=ip,i_beam=i_beam,
