...
while time < round(((extra_variables['timebase'])[-1]),3):  
    time = round(((extra_variables['timebase'])[j]),3) 
    X_data.append([(data['beta_N'])[j],((data['nebar_efit'])[j]*np.pi*0.22*0.22*(10.**-20.)/np.abs(0.000001*(data['ip'])[j])),(data['beta_p'])[j],
                        (data['li'])[j],(((data['rmag'])[j]-0.68)/0.22)])
    j = j + 1
    
#Loading saved model
RF_LHI_pkl_filename = '/home/mathewsa/Desktop/00010_RF_classifier_LHI.pkl'
RF_LHI_model_pkl = open(RF_LHI_pkl_filename, 'rb')
RF_LHI_model = pickle.load(RF_LHI_model_pkl)
NN_LHI_pkl_filename = '/home/mathewsa/Desktop/00010_NN_classifier_LHI.pkl'
NN_LHI_model_pkl = open(NN_LHI_pkl_filename, 'rb')
NN_LHI_model = pickle.load(NN_LHI_model_pkl)
GNB_LHI_pkl_filename = '/home/mathewsa/Desktop/00010_NB_classifier_LHI.pkl'
GNB_LHI_model_pkl = open(GNB_LHI_pkl_filename, 'rb')
GNB_LHI_model = pickle.load(GNB_LHI_model_pkl)
LR_LHI_pkl_filename = '/home/mathewsa/Desktop/00010_LR_classifier_LHI.pkl'
LR_LHI_model_pkl = open(LR_LHI_pkl_filename, 'rb')
LR_LHI_model = pickle.load(LR_LHI_model_pkl)

scalerfile = '/home/mathewsa/Desktop/00010_scaler.sav'
...
plt.xlabel(r'time (s)')
plt.ylim([-0.1,1.1]) 
plt.axvspan(1.53, 1.8,facecolor='whitesmoke', alpha=0.5)
plt.axvspan(0.85, 1.53, facecolor='orange', alpha=0.1)
plt.axvspan(0.0,0.85, facecolor='whitesmoke', alpha=0.5)
...
plt.show()
plt.figure()
plt.plot(timebase,(data['beta_N']),color='green')
plt.ylabel(r"$\beta_N$")
plt.xlabel(r'Time (s)')
plt.show()
plt.figure()
plt.plot(timebase,(data['nebar_efit']*np.pi*0.22*0.22*(10.**-20.)/np.abs(0.000001*data['ip'])),color='green') 
plt.ylabel(r"$n/n_G$")
plt.xlabel(r'Time (s)')
plt.show()
plt.figure()
...
plt.show()
plt.figure()
plt.plot(timebase,(((data['rmag'])-0.68)/0.22),color='green')
plt.ylabel(r"$\Delta$")
plt.xlabel(r'Time (s)')
plt.show()
plt.figure()
...
while i < len(rows):  
    if (values['ip'][i] != None) and (values['btor'][i] != None) and (values['Wmhd'][i] != None) and (values['nebar_efit'][i] != None) and (values['beta_p'][i] != None) and (values['P_ohm'][i] != None) and (values['li'][i] != None) and (values['rmag'][i] != None) and (values['Halpha'][i] != None) and (values['psurfa'][i] != None):
        ...
        X_data0.append([(values['shot'])[i],(values['beta_N'])[i],((values['nebar_efit'])[i]*np.pi*0.22*0.22*(10.**-20.)/np.abs(0.000001*(values['ip'])[i])),(values['beta_p'])[i],
...
beta_N_min, beta_N_max = X_data_normalized[:, 0].min(), X_data_normalized[:, 0].max()
n_nG_min, n_nG_max = X_data_normalized[:, 1].min(), X_data_normalized[:, 1].max()
beta_p_min, beta_p_max = X_data_normalized[:, 2].min(), X_data_normalized[:, 2].max()
li_min, li_max = X_data_normalized[:, 3].min(), X_data_normalized[:, 3].max()
Delta_min, Delta_max = X_data_normalized[:, 4].min(), X_data_normalized[:, 4].max()

#these are the actual true min values
true_values_min = scaler.inverse_transform([beta_N_min, n_nG_min, beta_p_min,\
    li_min, Delta_min])
true_values_max = scaler.inverse_transform([beta_N_max, n_nG_max, beta_p_max,\
    li_max, Delta_max])
    
#insert true values I want in here
min_beta_N = true_values_min[0] # ~= min(X_data0[:,0])
min_n_nG = 0.#true_values_min[1]
min_beta_p = 0.#true_values_min[2] 
min_li = true_values_min[3]
min_Delta = true_values_min[4] 

max_beta_N = true_values_max[0] # ~= min(X_data0[:,0])
max_n_nG = 1.5#true_values_max[1]
max_beta_p = true_values_max[2] 
max_li = true_values_max[3]
max_Delta = true_values_max[4] 

#these are scaled inputs of values I want
beta_N_min = scaler.transform([min_beta_N,min_n_nG, min_beta_p, min_li, min_Delta])[0]
n_nG_min = scaler.transform([min_beta_N,min_n_nG, min_beta_p, min_li, min_Delta])[1]
beta_p_min = scaler.transform([min_beta_N,min_n_nG, min_beta_p, min_li, min_Delta])[2]
li_min = scaler.transform([min_beta_N,min_n_nG, min_beta_p, min_li, min_Delta])[3]
Delta_min = scaler.transform([min_beta_N,min_n_nG, min_beta_p, min_li, min_Delta])[4]

beta_N_max = scaler.transform([max_beta_N,max_n_nG, max_beta_p, max_li, max_Delta])[0]
n_nG_max = scaler.transform([max_beta_N,max_n_nG, max_beta_p, max_li, max_Delta])[1]
beta_p_max = scaler.transform([max_beta_N,max_n_nG, max_beta_p, max_li, max_Delta])[2]
li_max = scaler.transform([max_beta_N,max_n_nG, max_beta_p, max_li, max_Delta])[3]
Delta_max = scaler.transform([max_beta_N,max_n_nG, max_beta_p, max_li, max_Delta])[4]
...
#in this example, the following 2 features will be plotted on contour map
#while the other features will be set at a constant
beta_N_constant = 0.65
n_nG_constant = 0.
beta_p_constant = 0. 
li_constant = 1.15
Delta_constant = 0.04

inputs_normalized = scaler.transform([beta_N_constant,n_nG_constant,beta_p_constant,\
                li_constant,Delta_constant]) 

x_input_, y_input_ = np.meshgrid(np.linspace(n_nG_min, n_nG_max, grid_steps),
                     np.linspace(beta_p_min, beta_p_max, grid_steps))
                     
n_nG_input = x_input_.ravel() #these are not constant
beta_p_input = y_input_.ravel() #these are not constant

beta_N_input = inputs_normalized[0]*np.ones(grid_steps*grid_steps) 
li_input = inputs_normalized[3]*np.ones(grid_steps*grid_steps)
Delta_input = inputs_normalized[4]*np.ones(grid_steps*grid_steps)


number_ticks = 9
y_labels = list(np.around(np.linspace(min_beta_p,max_beta_p,number_ticks),decimals=2))
x_labels = list(np.around(np.linspace(min_n_nG,max_n_nG,number_ticks),decimals=2))
...
y_description = r"$\beta_p$"
x_description = r"$n/n_G$"
...
for idx, clf, tt in zip(product([0, 1], [0, 1]),
    ...
    Z = clf.predict(np.c_[beta_N_input, n_nG_input, beta_p_input,
                          li_input, Delta_input])
...
for idx, clf, tt in zip(product([0, 1], [0, 1]),
    ...
    probs1 = clf.predict_proba(np.c_[beta_N_input, n_nG_input, beta_p_input,
                          li_input, Delta_input])[:, 1].reshape((grid_steps,grid_steps))
    probs2 = clf.predict_proba(np.c_[beta_N_input, n_nG_input, beta_p_input,
                          li_input, Delta_input])[:, 2].reshape((grid_steps,grid_steps))
...
f.autofmt_xdate()  
f.suptitle((r"$\beta_N = $"+y_fmt(beta_N_constant, 1)+
#             r", $P_{ohm} = $"+y_fmt(P_ohm_constant, 1)+r"$W$"
             r"$,\ l_{i} = $"+y_fmt(li_constant, 1)+ 
             r"$,\ \Delta = $"+y_fmt(Delta_constant, 1)),fontsize=25)
plt.show()
