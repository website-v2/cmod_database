...
while q < len(Y_data):
    if (Y_data[q] == '1') or (Y_data[q] == 1):
        p = p + 1
    q = q + 1
print('H-mode fraction to total dataset time slices: ',p,'/',len(Y_data))
...
while train_valid_frac < 0.99:
    print(train_valid_frac,'NEW')
    ...
    data = np.insert(X_train, len(X_train[0]), values=y_train, axis=-1)
    random = (np.random.permutation(data))
    X_train = random[:,:-1]
    y_train = random[:,-1]
...
#        j = j + 1
    ...
    plt.show()
    train_valid_frac = train_valid_frac + 0.05