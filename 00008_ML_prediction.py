...
#in this example, the following 2 features will be plotted on contour map
#while the other features will be set at a constant
Wmhd_constant = 120000.
nebar_efit = 0.
beta_p_constant = 0.
P_ohm_constant = 1500000.
li_constant = 1.10
rmag_constant = 0.68
Halpha_constant = 2.00

inputs_normalized = scaler.transform([Wmhd_constant,nebar_efit,beta_p_constant,\
                P_ohm_constant,li_constant,rmag_constant,Halpha_constant]) 

x_input_, y_input_ = np.meshgrid(np.linspace(nebar_efit_min, nebar_efit_max, grid_steps),
                                 np.linspace(beta_p_min, beta_p_max, grid_steps))
                     
beta_p_input = y_input_.ravel() #these are not constant
nebar_efit_input = x_input_.ravel() #these are not constant

Wmhd_input = inputs_normalized[0]*np.ones(grid_steps*grid_steps)
P_ohm_input = inputs_normalized[3]*np.ones(grid_steps*grid_steps)
li_input = inputs_normalized[4]*np.ones(grid_steps*grid_steps)
rmag_input = inputs_normalized[5]*np.ones(grid_steps*grid_steps)
Halpha_input = inputs_normalized[6]*np.ones(grid_steps*grid_steps)
...
x_labels = list(np.around(np.linspace(min_nebar_efit,max_nebar_efit,number_ticks),decimals=2))
x_ticks = (np.linspace(np.min(x_input_),np.max(x_input_),number_ticks))         
y_ticks = (np.linspace(np.min(y_input_),np.max(y_input_),number_ticks)) 
y_description = r"$\beta_p$"
x_description = r"$\bar{n} \ (m^{-3})$"
...
cbar2.ax.set_title(r"$\bf{H-mode}$", y=1.01, rotation=0) 
f.autofmt_xdate()  
f.suptitle((r"$W_{mhd} = $"+y_fmt(Wmhd_constant, 2)+r"$J$"
             r", $P_{ohm} = $"+y_fmt(P_ohm_constant, 2)+r"$W$"
             r"$,\ l_{i} = $"+y_fmt(li_constant, 2)+
             r"$,\ r_{mag} = $"+y_fmt(rmag_constant, 2)+"$m$"
             r"$,\ H_{\alpha} = $"+y_fmt(Halpha_constant, 2)+"$\\frac{W}{m^2 sr}$"),fontsize=25)
plt.show()
